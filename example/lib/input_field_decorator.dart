...
class InputFieldDecorator extends StatefulWidget {
  ...
  ///
  /// Null [InputFieldDecoration] properties are initialized with the corresponding
  /// values from [ThemeData.inputDecorationTheme].
  ///
  /// Must not be null.
  final InputFieldDecoration decoration;
  ...
  @override
  State<InputFieldDecorator> createState() => _InputFieldDecoratorState();
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<InputFieldDecoration>('decoration', decoration));
...
class _InputFieldDecoratorState extends State<InputFieldDecorator> {
  @override
...
class InputFieldDecoration {
  ...
  const InputFieldDecoration.collapsed({
  ...
  InputFieldDecoration applyDefaults(InputFieldDecorationTheme theme) {
    return copyWith(
  ...
  bool operator ==(Object other) {
    ...
    return other is InputFieldDecoration &&
        other.icon == icon &&
...
class InputFieldDecorationTheme with Diagnosticable {
  ...
  const InputFieldDecorationTheme({
    this.labelStyle,
    this.floatingLabelStyle,
    this.helperStyle,
    this.helperMaxLines,
  ...
  /// new values.
  InputFieldDecorationTheme copyWith({
    TextStyle? labelStyle,
    TextStyle? floatingLabelStyle,
    TextStyle? helperStyle,
    int? helperMaxLines,
    ...
    return InputFieldDecorationTheme(
      labelStyle: labelStyle ?? this.labelStyle,
      floatingLabelStyle: floatingLabelStyle ?? this.floatingLabelStyle,
      helperStyle: helperStyle ?? this.helperStyle,
      helperMaxLines: helperMaxLines ?? this.helperMaxLines,
...
class _RenderDecoration extends RenderBox with SlottedContainerRenderObjectMixin<_DecorationSlot> {
  ...
  _RenderDecorationLayout _layout(BoxConstraints layoutConstraints) {
    ...
    final double inputWidth = math.max(
      ...
      constraints.maxWidth -
          (_boxSize(icon).width + contentPadding.left + _boxSize(prefixIcon).width + _boxSize(prefix).width + _boxSize(suffix).width + _boxSize(suffixIcon).width + contentPadding.right),
    );
    ...
    final double labelWidth = math.max(
      0.0,
      constraints.maxWidth - (_boxSize(icon).width + contentPadding.left + _boxSize(prefixIcon).width + suffixIconWidth + contentPadding.right),
    );
    ...
    final double topInputBaseline = contentPadding.top + topHeight + inputInternalBaseline + baselineAdjustment + interactiveAdjustment;
    final double maxContentHeight = containerHeight - contentPadding.top - topHeight - contentPadding.bottom;
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    final Offset densityOffset = decoration.visualDensity.baseSizeAdjustment;
    final double contentHeight = contentPadding.top + (label == null ? 0.0 : decoration.floatingLabelHeight) + inputMaxHeight + contentPadding.bottom + densityOffset.dy;
  ...
  void performLayout() {
    ...
    if (label != null) {
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          decoration.borderGap.start = lerpDouble(labelX + _boxSize(label).width, _boxSize(container).width / 2.0 + floatWidth / 2.0, floatAlign);
        ...
        case TextDirection.ltr:
          ...
          decoration.borderGap.start = lerpDouble(labelX - _boxSize(icon).width, _boxSize(container).width / 2.0 - floatWidth / 2.0, floatAlign);
          break;
      }
  ...
  void paint(PaintingContext context, Offset offset) {
    void doPaint(RenderBox? child) {
      if (child != null) {
        print(_boxParentData(child).offset);
    ...
    if (label != null) {
      ...
      _labelTransform = Matrix4.identity()
        ..translate(dx, t == 0 ? labelOffset.dy + dy : labelHeight * scale)
        ..scale(scale);
